...
public class PositionIntense {
	...
	public PositionIntense(char[][] board, char turn) {
		this.board = board;
		this.turn = turn;
	}
	...
	public PositionIntense move(int[] idx) {
		char[][] newBoard = board.clone();
		newBoard[idx[0]][idx[1]] = turn;
		return new PositionIntense(newBoard, turn == 'x' ? 'o' : 'x');
	}

	/** Get next best move for computer. Return int[2] of {row, col} */
	int[] best_move() {
		int[] result = minimax(5, Integer.MIN_VALUE, Integer.MAX_VALUE);
...
//		}
		...
		if (nextMoves.isEmpty() || depth == 0) {
			...
			for (int[] move : nextMoves) {
				...
				if (turn == 'o') { // mySeed (computer) is maximizing player
				...
				} else { // comp is minimizing player
					score = minimax(depth - 1, alpha, beta)[0];
					if (score < beta) {
						beta = score;
						bestRow = move[0];
			...
			return new int[] { (turn == 'o') ? alpha : beta, bestRow, bestCol };
		}
	}
	...
	private int[] winningPatterns = { 0b1111000000000000, 0b0000111100000000, 0b0000000011110000, 0b0000000000001111, // rows
	...
	public boolean win(char thePlayer) {
		int pattern = 0b0000000000000000; // 9-bit pattern for the 9 cells
		for (int row = 0; row < ROWS; ++row) {
			for (int col = 0; col < COLS; ++col) {
